...
func TestIT_RoomController_PostMessageForRoom_WhenMessageHasWrongSyntax_ExpectBadRequest(t *testing.T) {
	service, _ := newTestRoomService(t)
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader("not-a-message-dto-request"))
	ctx, rw := generateTestEchoContextFromRequest(req)

	err := postMessageForRoom(ctx, service)
	assert.Nil(t, err, "Actual err: %v", err)
...
func TestIT_RoomController_PostMessageForRoom_WhenRoomHasEmptyName_ExpectBadRequest(t *testing.T) {
	service, _ := newTestRoomService(t)
...
func TestIT_RoomController_PostMessageForRoom(t *testing.T) {
	service, dbConn := newTestRoomService(t)
	defer dbConn.Close(context.Background())
