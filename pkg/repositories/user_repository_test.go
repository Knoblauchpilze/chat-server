...
func TestIT_UserRepository_DeleteUserFromRooms(t *testing.T) {
	repo, conn, tx := newTestUserRepositoryAndTransaction(t)
	defer conn.Close(context.Background())
	...
	err := repo.DeleteFromRooms(context.Background(), tx, user.Id)
	tx.Close(context.Background())
...
func TestIT_UserRepository_DeleteUserFromRooms_DoesNotDeleteOtherUsers(t *testing.T) {
	repo, conn, tx := newTestUserRepositoryAndTransaction(t)
	defer conn.Close(context.Background())
	...
	err := repo.DeleteFromRooms(context.Background(), tx, user1.Id)
	tx.Close(context.Background())
