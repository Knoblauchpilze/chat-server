...
func TestUnit_ConnectionAcceptor_ListenWithContext(t *testing.T) {
	ca := newWebsocketAcceptor(newTestAcceptorConfig(5200), logger.New(os.Stdout))
...
func TestUnit_ConnectionAcceptor_WhenStartedMultipleTimes_ExpectFailure(t *testing.T) {
	ca := newWebsocketAcceptor(newTestAcceptorConfig(5201), logger.New(os.Stdout))

	// Start a first time: it should run without error.
	...
	// Start a second time: this should fail.
	err := ca.Accept()
...
func TestUnit_ConnectionAcceptor_WhenNotStarted_StopDoesNotFail(t *testing.T) {
	ca := newWebsocketAcceptor(newTestAcceptorConfig(5202), logger.New(os.Stdout))
...
func TestUnit_ConnectionAcceptor_WhenPortIsNotFree_ExpectConstructionReturnsInitializationFailure(t *testing.T) {
	...
	ca1 := newWebsocketAcceptor(newTestAcceptorConfig(5203), log)
	ca2 := newWebsocketAcceptor(newTestAcceptorConfig(5203), log)
...
func TestUnit_ConnectionAcceptor_WhenPortIsNotFree_ExpectStopDoesNotCrash(t *testing.T) {
	log := logger.New(os.Stdout)
	ca1 := newWebsocketAcceptor(newTestAcceptorConfig(5204), log)
	wg := asyncRunAcceptorAndWaitForItToBeUp(t, ca1)
	ca2 := newWebsocketAcceptor(newTestAcceptorConfig(5204), log)
...
func TestUnit_ConnectionAcceptor_WhenAcceptorIsStopped_ExpectConnectionToNotBeClosed(t *testing.T) {
	ca := newWebsocketAcceptor(newTestAcceptorConfig(5205), logger.New(os.Stdout))
...
func TestUnit_ConnectionAcceptor_ConnectDisconnect(t *testing.T) {
	ca := newWebsocketAcceptor(newTestAcceptorConfig(5206), logger.New(os.Stdout))
...
func TestUnit_ConnectionAcceptor_OnConnect_ExpectCallbackNotified(t *testing.T) {
	...
	var called int
	config.Callbacks.ConnectCallback = func(conn *websocket.Conn) {
		called++
	}

	ca := newWebsocketAcceptor(config, logger.New(os.Stdout))
...
func asyncRunAcceptorAndWaitForItToBeUp(
	t *testing.T, ca websocketAcceptor,
) *sync.WaitGroup {
	var wg sync.WaitGroup
...
func closeAcceptorAndAssertNoError(t *testing.T, ca websocketAcceptor, wg *sync.WaitGroup) {
	err := ca.Close()
	wg.Wait()
	assert.Nil(t, err, "Actual err: %v", err)
}
